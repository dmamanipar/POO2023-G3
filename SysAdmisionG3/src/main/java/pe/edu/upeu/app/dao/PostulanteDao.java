...
public class PostulanteDao implements PostulanteDaoI {
    ...
    public List<PostulanteTO> listarTodo() {
        ...
        String sql = "SELECT po.*, p.nombre as nombreperiodo, c.nombrecarreraFROM"
                + " postulante po, periodo p, carrera c WHERE p.id_periodo = po.id_periodo "
                + "and po.id_carrera = c.id_carrera";
        try {
            ps = connection.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
        ...
        do {
            switch (opc) {
                case "C" -> {
                    ...
                    d.setIdCarrera(sc.next());
                    System.out.println("Ingrese Periodo:");
                    d.setIdPeriodo(sc.next());
                    dao.create(d);
    ...
    public void listarPostulantes(List<PostulanteTO> Lista) {
        ...
        for (PostulanteTO po : Lista) {
            System.out.println(po.getDni() + "\t" + po.getNombre() + "\t" + po.getApellidoMat());
        }
    }
    ...
    public PostulanteTO buscarEntidad(String dni) {
        ...
        String sql = "SELECT po.*, p.nombre as nombreperiodo, "
                + "c.nombrecarrera FROM postulante po, periodo p, "
                + "carrera c WHERE po.dni =  ? and p.id_periodo = "
                + "po.id_periodo and po.id_carrera = c.id_carrera";
        try {
//connection = new Conn().connectSQLite();
            ...
            if (rs.next()) {
                ...
                entidad.setNombreModalidad(buscarModalidadExamen(rs.getString("modalidad")));
            }
        } catch (SQLException e) {
            System.out.println(e.toString());
        }
